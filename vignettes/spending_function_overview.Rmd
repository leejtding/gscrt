...
bibliography: gs_design_crt.bib
nocite: |
  @JTBook
  @gsDesign
vignette: >
...
Normally a spending function will be passed to `gs_design_crt()` in the parameter
`alpha_sf` for the upper bound and `beta_sf` for the lower bound to specify a
...
See `spending_function()` for the input and output specification of
