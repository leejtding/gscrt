...
void gs_lower_2(int *x_n_anal,double *x_theta,double *I,double *a,double *b,double *prob_lo,
    ...
    double plo=0.,phi,dplo,atem=0.,atem2,rt_delta_k,rt_ik,rt_ikm1,xlo,xlo2,xhi,theta,mu,tol,adelta;
    ...
    else atem2=qnorm(prob_lo[0],mu,1.,1,0);
    adelta=1.; j=0;
    ...
    {   rt_ikm1=rt_ik; rt_ik=sqrt(I[i]); mu=rt_ik*theta; rt_delta_k=sqrt(I[i]-I[i-1]);
        ...
        else atem2=qnorm(prob_lo[i],mu,1.,1,0); 
        adelta=1.; j=0;
		...
		{   phi=0.; dplo=0.; plo=0.;
            ...
            for(ii=0;ii<=m1;ii++)
            {   xhi=(z1[ii]*rt_ikm1-b[i]*rt_ik+theta*(I[i]-I[i-1]))/rt_delta_k;
                phi += pnorm(xhi,0.,1.,1,0)*h[ii];

				xlo=(-z1[ii]*rt_ikm1+atem*rt_ik-theta*(I[i]-I[i-1]))/rt_delta_k;
                xlo2=(-z1[ii]*rt_ikm1-atem*rt_ik-theta*(I[i]-I[i-1]))/rt_delta_k;
				plo += h[ii]*(pnorm(xlo,0.,1.,1,0)-pnorm(xlo2,0.,1.,1,0));
                dplo+=h[ii]*(rt_ik/rt_delta_k)*(exp(-xlo*xlo/2)+exp(-xlo2*xlo2/2))/rt_2pi;
            ...
            if (DEBUG) Rprintf("i=%2d j=%2d plo=%lf atem=%lf phi=%lf dplo=%lf\n",i,j,plo,atem,phi,dplo);
