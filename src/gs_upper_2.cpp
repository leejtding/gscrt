...
void gs_upper_2(int *x_n_anal,double *x_theta,double *I,double *a,double *b,double *prob_lo,
    ...
    double plo=0.,phi,dphi,btem=0.,btem2,rt_delta_k,rt_ik,rt_ikm1,xlo,xlo2,xhi,theta,mu,tol,bdelta;
    ...
    prob_lo[0]=pnorm(a[0]-mu,0.,1.,1,0) - pnorm(-a[0]-mu,0.,1.,1,0);
    ...
    {   rt_ikm1=rt_ik; rt_ik=sqrt(I[i]); mu=rt_ik*theta; rt_delta_k=sqrt(I[i]-I[i-1]);
		...
		{   phi=0.; dphi=0.; plo=0.;
            ...
            {   xhi=(z1[ii]*rt_ikm1-btem*rt_ik+theta*(I[i]-I[i-1]))/rt_delta_k;
                ...
                xlo2=(-z1[ii]*rt_ikm1-a[i]*rt_ik-theta*(I[i]-I[i-1]))/rt_delta_k;
				plo += (pnorm(xlo,0.,1.,0,0)-pnorm(xlo2,0.,1.,0,0))*h[ii];
