...
void gs_bounds_1(int *x_n_anal, double *x_theta, double *I, double *a, double *b,
    ...
    double plo,phi,dplo,dphi,btem=0.,atem=0.,atem2,btem2,rt_delta_k,rt_ik,rt_ikm1,xlo,xhi,theta,mu1,mu2;
    ...
    {   retval[0]=1;
        if (*printerr)
        {	Rprintf("gs_bounds_1 error: illegal argument");
    ...
    else a[0]=qnorm(prob_lo[0],mu2,1.,1,0);
    ...
    m11=grid_pts(r,mu1,a[0],b[0],z11,w11);
    h_1(0.,m11,w11,I[0],z11,h11);
    
    m12=grid_pts(r,mu2,a[0],b[0],z12,w12);
    ...
    {   /* set up constants */
        ...
        if (rt_delta_k < 1) rt_delta_k=1;
        if (prob_lo[i]<=0.) atem2= -EXTREME_Z;
        else atem2=qnorm(prob_lo[i],mu2,1.,1,0); 
	    ...
	    {   plo=0.; dplo=0.; atem=atem2;
            ...
            {   xlo=(z12[i2]*rt_ikm1-atem*rt_ik+theta*(I[i]-I[i-1]))/rt_delta_k;
                ...
                dplo+=h12[i2]*exp(-xlo*xlo/2)/rt_2pi*rt_ik/rt_delta_k;
            }
        ...
        if (i<n_anal-1)
        {   m21=grid_pts(r,mu1,a[i],b[i],z21,w21);
            m22=grid_pts(r,mu2,a[i],b[i],z22,w22);
